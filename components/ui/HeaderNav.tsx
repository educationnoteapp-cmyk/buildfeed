...
export default function HeaderNav() {
  ...
  if (!user) {
    return (
      <nav className="flex items-center gap-3">
        ...
        <Link href="/login" className="text-sm px-4 py-1.5 bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors font-medium">
          Studio →
        </Link>
      </nav>
    )
  ...
  return (
    <nav className="flex items-center gap-3" ref={menuRef}>
      <Link href="/studio"
        className="text-sm px-4 py-1.5 bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors font-medium">
        Studio →
      </Link>

      {/* Avatar + dropdown */}
      <button onClick={() => setOpen(o => !o)}
        className="relative w-8 h-8 rounded-full overflow-hidden border-2 border-border hover:border-primary/50 transition-colors flex-shrink-0 bg-primary/20 flex items-center justify-center">
        {user.avatar_url
      ...
      {open && (
        <div className="absolute top-14 left-4 right-4 sm:left-auto sm:right-4 sm:w-48 bg-surface border border-border rounded-2xl shadow-xl z-50 overflow-hidden">
          ...
          <div className="py-1">
            <Link href={`/creator/${user.username}`} onClick={() => setOpen(false)}
              className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-text-main hover:bg-white/5 transition-colors">
              <User size={14} className="text-muted" /> פרופיל שלי
            </Link>
            ...
            <Link href="/settings" onClick={() => setOpen(false)}
              className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-text-main hover:bg-white/5 transition-colors">
              <Settings size={14} className="text-muted" /> הגדרות
            </Link>
